...
class CoupleService {
    async handlePairing(userId, partnerCode) {
        ...
        if (partner.partner_code === currentUser.unique_code) {
            ...
            // Update partner's partner_code to complete the reciprocal link
            await supabase.from('users').update({ partner_code: currentUser.unique_code }).eq('id', partner.id);
            ...
            // Return partner info for frontend
            return { 
                status: 'paired', 
                couple,
                partner: {
